...
class Vilao(Personagem):
    ...
    def apply_condicao(self) -> str:
        ...
        if self.condicao == "Paralisado": # O vilão tem uma chance de 1/3 de não atacar quando paralisado
            self.idle = False

            if choice([True, False, False]):
                self.idle = True
                log += f'{self.nome} está paralisado e não conseguiu atacar!\n'
        elif self.condicao == "Congelado": # O vilão não ataca quando congelado, mas irá se recuperar dessa condição no terceiro turno
            self.idle = True
        ...
        elif self.condicao == "Queimado": # O vilão perde 2,5% ou 7,5% de sua vida a cada turno quando queimado
            self.downgrade_vida(self.vida_max * choice([0.025, 0.075]))
